...
下面这个教程将通过一个名为 MPLbank 的模拟零售银行，介绍 IBM Bluemix 上发布的银行 API 和在 IBM Z 大型机上运行的逻辑业务程序。

# MPLbank

## 架构

本教程将访问一个名为 MPLbank 的虚构零售银行系统。MPLbank 集成了一个帐户管理系统、一个支付中心（图解中未显示）、一个客户管理系统和一个金融风险管理系统。基于这些组件，我们设置了一个托管在 IBM Bluemix 中的 API 层来提供一个银行 API，并使各种各样的应用程序能够访问它。
...
客户数据库系统和金融风险管理系统通过内置的 REST/JSON 接口来公开服务。在帐户管理系统前面，设置了 IBM z/OS Connect EE 来创建与 CICS 中运行的 COBOL 程序的 REST/JSON 接口。因此，这 3 个子系统非常灵活且可使用 REST/JSON API 进行访问。这些子系统充当着微服务来提供 REST/JSON 接口，IBM API Connect 充当着一个 API 层来管理一组唯一且最终的银行 REST/JSON API。得到的混合架构方法吸收了两种架构的优势，打造了一种快速且安全的 API 经济。

有关数据的更多信息请查阅 [MPLbank 自述文件]。

本教程的目的是使用一个示例银行应用程序来发现、测试和使用这个银行 API，然后使用 IBM Bluemix 增强它。

## 包含的组件

本旅程使用一种混合 [IBM Bluemix] / [IBM Z 大型机]架构来完成。
...
4. 	测试使用您的编程语言的 **GET /customers/{customerID}** 操作。
	...
	* 下滚到 **Try this operation** 部分。
	...
	> 重要事项：这个 Github 存储库中的 */identifier/customerIDs.txt* 文件中提供了所有可用的客户 ID。不要忘记在 *x-ibm-client-id* 和 *x-ibm-client-secret* 中填入您的信息。
...
5.	重放此场景，以测试这个银行 API 中的所有操作。可以在 **GET /customers/{customerID}** 操作请求的 JSON 输出中找到其他操作输入参数。
...
3.	检查 *bankingAPI.js* 文件，以理解该脚本的工作原理。
	...
	* 如果需要，更改客户 ID。这段示例代码中已硬编码了客户 ID。 
	> 备注：可以通过 this Github repository 中的 *identifier* 文件夹更改客户标识符。生成了 1000 个客户，这意味着它表示 1000 个不同的银行客户。
	* 将 **IBM_CLIENT_ID** 和 **IBM_CLIENT_SECRET** 变量替换为您的信息并保存该文件。
	
## 2.运行银行应用程序
	
1.	在最喜欢的 Web 浏览器中打开 *index.html*。该应用程序将自动运行。
...
4.	调用示例银行应用程序中的其他每个服务。每个服务与一个已发布的银行操作匹配：
	* 	**Customer contract**
...
银行应用程序现在已托管在 Bluemix 中并使用了银行 API。
...
3. 	等待所选的 Watson 服务创建完成。完成后将提示重新暂存您的 Node.js 应用程序。确认重新暂存。

4.	在您的 Node.js 应用程序中探索所选的 Watson 服务
